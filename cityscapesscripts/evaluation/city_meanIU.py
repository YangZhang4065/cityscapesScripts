...
def evaluatePair_image(predictionNp, groundTruthNp, confMatrix, instanceStats, perImageStats,i_im, args):
    # Loading all resources for evaluation.
    ...
    if args.evalPixelAccuracy:
        ...
        erroneousPixels = np.logical_and( notIgnoredPixels , ( predictionNp != groundTruthNp ) )
        perImageStats[i_im] = {}
        perImageStats[i_im]["nbNotIgnoredPixels"] = np.count_nonzero(notIgnoredPixels)
        perImageStats[i_im]["nbCorrectPixels"]    = np.count_nonzero(erroneousPixels)

    return nbPixels

# The main method
#def main(argv,groundTruthImgList,predictionImgList):
#    global args
#
#    # evaluate
...
#    return [path+f for f in file_list if isfile(join(path, f))]
#GT_list=return_list('/home/gong/crcv/home/yangz/segmentation_py/dataset/prediction/Parsed_CityScape_val_new/GT/')
#main(sys.argv[1:],GT_list,return_list('/home/gong/crcv/home/yangz/segmentation_py/dataset/prediction/Parsed_CityScape_val_new_v2/MMD_FCN_DA_trained_local_new_class_v2/'))




def convert_classes(annot):
    new_annot=np.copy(annot)
    new_annot[annot==255]=0
    new_annot[annot==1]=23
    new_annot[annot==2]=11
    new_annot[annot==3]=7
    new_annot[annot==4]=8
    new_annot[annot==5]=13
    new_annot[annot==6]=21
    new_annot[annot==7]=17
    new_annot[annot==8]=26
    new_annot[annot==9]=20
    new_annot[annot==10]=24
    new_annot[annot==11]=33
    new_annot[annot==12]=32
    new_annot[annot==15]=19
    new_annot[annot==17]=25
    new_annot[annot==19]=28
    new_annot[annot==21]=12
    return new_annot
    
def city_meanIU(Ytrue_in,Ypred_in):
    global args
    
    Ytrue=convert_classes(Ytrue_in)
    Ypred=convert_classes(Ypred_in)
    confMatrix    = generateMatrix(args)
    instStats     = generateInstanceStats(args)
    perImageStats = {}
    nbPixels      = 0
    ...
    for i in range(Ytrue.shape[0]):
        predictionNp = np.squeeze(Ypred[i,:,:])
        groundTruthNp = np.squeeze(Ytrue[i,:,:])
        #print "Evaluate ", predictionImgFileName, "<>", groundTruthImgFileName
        nbPixels += evaluatePair_image(predictionNp, groundTruthNp, confMatrix, instStats, perImageStats,i, args)
...
def COW_convert_classes(annot):
    new_annot=np.copy(annot)
    new_annot[annot==255]=0
    new_annot[annot==0]=12
    new_annot[annot==1]=23
    new_annot[annot==2]=11
    new_annot[annot==3]=7
    new_annot[annot==4]=8
...
def COW_meanIU(Ytrue_in,Ypred_in):
    global args
    
    args.quiet=True
    
    Ytrue=COW_convert_classes(Ytrue_in)
    Ypred=COW_convert_classes(Ypred_in)
    confMatrix    = generateMatrix(args)
    instStats     = generateInstanceStats(args)
    perImageStats = {}
    nbPixels      = 0
...
def city_meanIU_print(Ytrue_in,Ypred_in):
    global args
    args.quiet              = False
    Ytrue=convert_classes(Ytrue_in)
    Ypred=convert_classes(Ypred_in)
